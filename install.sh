...
bold=$(tput bold)
normal=$(tput sgr0)
refind_dir_default="/boot/EFI/refind"
#Set install path
...
read -p "${bold}1: light${normal}, 2: dark (oled): " theme_select
...
cd refind-theme-regular-oled
cp src/theme.conf theme.conf
sed -i "s/#icons_dir themes\/refind-theme-regular-oled\/icons\/$size_big-$size_small/icons_dir themes\/refind-theme-regular-oled\/icons\/$size_big-$size_small/" theme.conf
...
sed -i "s/#banner themes\/refind-theme-regular-oled\/icons\/$size_big-$size_small\/bg$theme_path.png/banner themes\/refind-theme-regular-oled\/icons\/$size_big-$size_small\/bg$theme_path.png/" theme.conf
sed -i "s/#selection_big themes\/refind-theme-regular-oled\/icons\/$size_big-$size_small\/selection$theme_path-big.png/selection_big themes\/refind-theme-regular-oled\/icons\/$size_big-$size_small\/selection$theme_path-big.png/" theme.conf
sed -i "s/#selection_small themes\/refind-theme-regular-oled\/icons\/$size_big-$size_small\/selection$theme_path-small.png/selection_small themes\/refind-theme-regular-oled\/icons\/$size_big-$size_small\/selection$theme_path-small.png/" theme.conf
cd ..
echo " - [DONE]"

#Clean up
echo -n "Removing unused directories"
rm -rf refind-theme-regular-oled/{src,.git}
rm -rf refind-theme-regular-oled/install.sh
echo " - [DONE]"

#Remove previous installs
echo -n "Deleting older installed versions (if any)"
rm -rf "${refind_dir}"/{regular-theme,refind-theme-regular,refind-theme-regular-oled}
rm -rf "${refind_dir}"/themes/{regular-theme,refind-theme-regular,refind-theme-regular-oled}
echo " - [DONE]"

#Copy theme setup folders
echo -n "Copying theme to ${refind_dir}/themes"
mkdir -p "${refind_dir}/themes"
cp -r refind-theme-regular-oled "${refind_dir}/themes"
echo " - [DONE]"
...
include themes/refind-theme-regular-oled/theme.conf" | tee -a "${refind_dir}"/refind.conf &> /dev/null
echo " - [DONE]"
